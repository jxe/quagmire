...
| `didNavigateBack` | `() -> Void` | Cmd-[ in nav mode (or Cmd-[ in edit mode — that path commits live text first). | Host pops its navigation stack. |
...
- **No multi-page navigation.** The editor is single-page; the host
  ...
  `didActivateLink(.workspacePage(...))` / `didNavigateBack()`. Hunch uses

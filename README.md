...
final class MyHost: EditorHost {
    ...
    func openLink(_ target: LinkTarget) -> Bool { false }
...
- **`Block.subpage`'s `pageID: String`** is opaque to the editor —
  ...
  The editor echoes it back unchanged in `openLink(.workspacePage(...))`,
  `onLoadSubpage`, `onAbsorbSubpage`, `onAppendToSubpage`.
...
| `documentDidChange` | `(_ ops: [EditorOp], _ on: Document) -> Void` | At every edit-session commit point — structural mutations via `EditorView.mutate(_:_:)`, typing commits (blur, focus change, navigation, scenePhase) via the `BlockTextEditor` coordinator's `commitLiveText` → `DocumentUndoController.afterCommit` hook, autotransforms, undo, paste, move-to. Called *synchronously* on the mutation-commit thread so the host's dirty flag is readable in immediate flush-on-close paths. When the change came through `mutate`, `ops` is the pre→post diff from `BlockTreeDiff.derive(_:_:)`: `.insert(hash, parent, block)` for new or content-changed blocks, `.remove(hash)` for hashes that are no longer the live hash of any post id. When the change came from a typing commit, `ops` is a single `[.remove(preHash), .insert(nowHash, …)]` pair diffing the active block's `EditorState.editingPreHash` (snapshotted at edit-session entry) against its current `atomicHash`. Empty `ops` means a pure reorder/move (same id, same hash) — the host should still persist the new tree shape. | Host should treat the call as the unit of save: apply non-empty `ops` to its recovery log, then write the rendered document, in that order. Fire-and-forget; default impl is a no-op. |

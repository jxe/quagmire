...
private extension NSAttributedString.Key {
    /// Marks a semibold font that exists only to present a link, rather than
    ...
    static let linkPresentationSemibold = NSAttributedString.Key("Quagmire.LinkPresentationSemibold")
}
...
enum InlineMarksBridge {
    ...
    static func toNS(_ source: AttributedString, baseFontSize: CGFloat, baseBold: Bool, lineSpacing: CGFloat, theme: EditorTheme = .default) -> NSAttributedString {
        ...
        for run in source.runs {
            ...
            let inlineBold = run[InlineAttributes.BoldAttribute.self] == true
            let bold = inlineBold || baseBold
    ...
    static func typingAttributes(
        ...
        case .bold:
            ...
            let italic = traits.contains(.italicTrait)
            let presentationOnly = attrs[.linkPresentationSemibold] as? Bool == true
